...
def ensure_clients_schema(cursor):
    table_info = cursor.execute("PRAGMA table_info(clients)").fetchall()
    if not table_info:
        return
    existing_columns = {row[1] for row in table_info}
    if "enabled" not in existing_columns:
        cursor.execute("ALTER TABLE clients ADD COLUMN enabled INTEGER NOT NULL DEFAULT 1")


def ensure_wg_interfaces_schema(cursor, *, print_fn, integrity_error_type):
    table_info = cursor.execute("PRAGMA table_info(wg_interfaces)").fetchall()
    if not table_info:
        return
    expected_columns = {
        "Jc": "INTEGER",
