...
test('firewall policy raw/mangle: context-specific controls are gated', async ({ page }) => {
  ...
  let activePanel = page.locator('[role="tabpanel"][data-state="active"]').last()
  ...
  await expect(activePanel.getByText('notrack (advanced mode)', { exact: true })).toHaveCount(0)
  await page.getByRole('tab', { name: 'Action' }).click()
  activePanel = page.locator('[role="tabpanel"][data-state="active"]').last()
  await expect(activePanel.getByText('nftrace', { exact: true })).toBeVisible()
  await expect(activePanel.getByText('notrack', { exact: true })).toBeVisible()
  await expect(activePanel.getByText('meta mark set')).toHaveCount(0)
  await expect(activePanel.getByText('NAT actions are shown only when the selected family/table/chain supports NAT.')).toBeVisible()
  await expect(activePanel.getByText('ct mark set')).toHaveCount(0)
  ...
  activePanel = page.locator('[role="tabpanel"][data-state="active"]').last()
  await expect(activePanel.getByText('Raw expression & debug')).toHaveCount(0)
  await expect(activePanel.getByText('nftrace (raw table only)', { exact: true })).toHaveCount(0)
  await expect(activePanel.getByText('notrack (raw table only)', { exact: true })).toHaveCount(0)
  await page.getByRole('tab', { name: 'Action' }).click()
  activePanel = page.locator('[role="tabpanel"][data-state="active"]').last()
  await expect(activePanel.getByText('nftrace (raw table only)', { exact: true })).toBeVisible()
  await expect(activePanel.getByText('notrack (raw table only)', { exact: true })).toBeVisible()
})
...
test('firewall walkthrough I: raw notrack and nftrace can be saved from UI', async ({ page, request }) => {
  ...
  try {
    ...
    let modalPanel = modal.locator('[role="tabpanel"][data-state="active"]').last()
    await expect(modalPanel.getByText('Raw expression & debug')).toHaveCount(0)
    await expect(modalPanel.getByText('nftrace (raw table only)', { exact: true })).toHaveCount(0)
    await expect(modalPanel.getByText('notrack (raw table only)', { exact: true })).toHaveCount(0)
    await modal.getByRole('tab', { name: 'Action' }).click()
    modalPanel = modal.locator('[role="tabpanel"][data-state="active"]').last()
    await expect(modalPanel.getByText('nftrace (raw table only)', { exact: true })).toBeVisible()
    await expect(modalPanel.getByText('notrack (raw table only)', { exact: true })).toBeVisible()
    ...
    modalPanel = modal.locator('[role="tabpanel"][data-state="active"]').last()
    await expect(modalPanel.getByText('Raw expression & debug')).toHaveCount(0)
    await expect(modalPanel.getByText('nftrace', { exact: true })).toHaveCount(0)
    await expect(modalPanel.getByText('notrack', { exact: true })).toHaveCount(0)
    await modal.getByRole('tab', { name: 'Action' }).click()
    await modal.getByLabel('nftrace').check()
    await modal.getByLabel('notrack').check()
