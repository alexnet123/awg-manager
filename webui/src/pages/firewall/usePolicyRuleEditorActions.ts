...
export function usePolicyRuleEditorActions(params: Params) {
  const onSave = React.useCallback(async (event: React.FormEvent) => {
    ...
    try {
      if (params.editingRuleId) await updateFirewallRule(params.auth, params.editingRuleId, params.form)
      else await createFirewallRule(params.auth, params.form)
  ...
  const openCreateWindow = React.useCallback(() => {
    params.setEditingRuleId(null)
    params.setRuleEditorTab('base')
    params.setAdvOpen({ ...params.advancedSectionsClosed })
    const ruleTable = params.activeRuleTableName
    params.setForm({ ...params.defaultRule, table: ruleTable, chain: params.activeChainOptions[0] || 'input' })
